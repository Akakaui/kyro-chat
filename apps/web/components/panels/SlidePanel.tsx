...
export function SlidePanel() {
  ...
  const loadProjects = async () => {
    try {
      ...
      if (res.projects && res.projects.length > 0) {
        ...
        setProjects([
          { id: "kyro-chat", name: "kyro chat", created_at: Date.now(), updated_at: Date.now() },
          { id: "akaka-portfolio", name: "c:\\Users\\Owner\\akaka-portf...", created_at: Date.now() - 86400000 * 5, updated_at: Date.now() - 86400000 * 5 },
          { id: "akaka", name: "akaka", created_at: Date.now() - 86400000 * 10, updated_at: Date.now() - 86400000 * 10 },
          { id: "klin", name: "klin", created_at: Date.now() - 86400000 * 12, updated_at: Date.now() - 86400000 * 12 },
          { id: "design-main", name: "design--main", created_at: Date.now() - 86400000 * 15, updated_at: Date.now() - 86400000 * 15 },
          { id: "vartex-portfolio", name: "vartex-architects-portfolio", created_at: Date.now() - 86400000 * 20, updated_at: Date.now() - 86400000 * 20 },
          { id: "users-owner", name: "C:\\Users\\Owner", created_at: Date.now() - 86400000 * 30, updated_at: Date.now() - 86400000 * 30 },
          { id: "gemini-anti", name: "c:\\Users\\Owner\\.gemini\\anti...", created_at: Date.now() - 86400000 * 40, updated_at: Date.now() - 86400000 * 40 },
        ])
      }
    } catch {
      setProjects([
  ...
  const handleCreateProject = async () => {
    ...
    try {
      ...
      const newP: Project = { id: `proj-${Date.now()}`, name: newProjectName.trim(), created_at: Date.now(), updated_at: Date.now() }
  ...
  const handleArtifactOpen = (artifact: typeof artifacts[0]) => {
  ...
  return (
    <>
      ...
      >
        ...
        <Tabs.Root value={activeSlideTab} onValueChange={(v) => setActiveSlideTab(v as "chats" | "projects" | "artifacts")}>
          <Tabs.List className="flex border-b border-[#18181f] shrink-0">
            <Tabs.Trigger
              value="chats"
              className={cn(
          ...
          {/* Chats Tab */}
          <Tabs.Content value="chats" className="flex-1 overflow-hidden data-[state=inactive]:hidden">
          ...
          {/* Projects Tab */}
          <Tabs.Content value="projects" className="flex-1 overflow-hidden data-[state=inactive]:hidden">
            <ScrollArea className="h-full px-3 py-3">
              <div className="space-y-4">
                {/* Project header */}
          ...
          {/* Artifacts Tab */}
          <Tabs.Content value="artifacts" className="flex-1 overflow-hidden data-[state=inactive]:hidden">
            <ScrollArea className="h-full px-3 py-3">
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-medium text-gray-400 tracking-tight">
                  ...
                  <span className="text-[11px] text-gray-500">{artifacts.length}</span>
                </div>

                {artifacts.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-12 text-center">
                    <Sparkles size={24} className="text-gray-600 mb-3" />
                  ...
                  <div className="space-y-1">
                    {artifacts.map((artifact) => {
                      ...
                      return (
                        <button
                          key={artifact.id}
                          onClick={() => handleArtifactOpen(artifact)}
                          className="group flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left transition-colors hover:bg-[#16161e]"
                        >
